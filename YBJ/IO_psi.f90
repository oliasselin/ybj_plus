...
!!! prep restart file (output) (define the variables and dims)
  ...
  call check( nf90_create("psi.out.ncf",ior(NF90_NETCDF4,NF90_MPIIO),idout, comm = MPI_COMM_WORLD,info = MPI_INFO_NULL) )
...
!  integer, dimension(5) :: ncstartr,ncstarti,nccount
  ...
  print*,'Yo! reading from netcdf restart file'
  call check( nf90_open('psi.in.ncf', NF90_NOWRITE,idin) )
