...
field_title = ['AR','AR_pred','AI','AI_pred','$\Re(LA)$','$\Re(LA_p)$','LAI','LAI_pred','test','$\Re(LA)$','betax']
field_name  = ['AR','AR_pred','AI','AI_pred','LAR','LAR_pred','LAI','LAI_pred','test','WKE_pred','betax']
...
BetaX  = np.zeros((gp_depth,gp_xrange))          #d2 Zeta'(z,x')/dx'2 --- gradient of gradient of vertical vorticity at y'=0 
...
for iz in range(gp_depth):
    ...
    bigD[iz,:]   = dz*gpz[iz]
    XP[iz,:]     = xp_m[:]
    BetaX[iz:]   = k_over_sqrt2*grad_zeta*np.sin(k_over_sqrt2*xp_m)
...
for ts in ts_plot:
    ...
    if os.path.isfile(path_ar):
        ...
        phi2_star = -phi_star*BetaX/(beta*beta*time)

        #decay = np.exp(-np.power(m_star*sigma_w,2)/4.)
        decay = np.exp(-np.power(m_star*sigma_w,2)/4.)*np.exp(phi2_star)
...
#        phase_simple2= -N2*beta*beta*np.power(time,3)/(24*cor*np.power(m_star,2))
        ...
        #Calculate the prediction for LA by taking f2/N2 (dz^2 A_pred)
#        phiz  = ii*(-N2*beta*beta*np.power(time,3)/(36*cor*m_star*m_star*ZZ) + (2./3.)*m_star ) + np.power(sigma_w*m_star,2)/(6.*ZZ)
#        phizz = ii*( N2*beta*beta*np.power(time,3)/(108*cor*np.power(m_star*ZZ,2)) - (2./9.)*m_star/ZZ ) - (5./18.)*np.power(sigma_w*m_star/ZZ,2)

        phiz  = ii*(-N2*beta*beta*np.power(time,3)/(36*cor*m_star*m_star*ZZ) + (2./3.)*m_star ) + np.power(sigma_w*m_star,2)/(6.*ZZ) - m_star*BetaX/(beta*beta*time)
        phizz = ii*( N2*beta*beta*np.power(time,3)/(108*cor*np.power(m_star*ZZ,2)) - (2./9.)*m_star/ZZ ) - (5./18.)*np.power(sigma_w*m_star/ZZ,2) + (m_star/3.*ZZ)*BetaX/(beta*beta*time)
...
#                    im = ax.imshow(Zprime,cmap=colormap,aspect=aspect)
                    im = ax.imshow(BetaX,cmap=colormap,aspect=aspect)
