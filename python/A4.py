...
field_title = ['AR','AR_pred','AI','AI_pred','$\Re(LA)$','$\Re(LA_p)$','LAI','LAI_pred','test','$\Re(LA)$','exp(kappa)','$\Re(LA)$ with E contours']
field_name  = ['AR','AR_pred','AI','AI_pred','LAR','LAR_pred','LAI','LAI_pred','test','LAR_pred','arg','E1']
...
ZetaP  = np.zeros((gp_depth,gp_xrange))          #Zeta'(z,x') --- vertical vorticity 
ZetaPP = np.zeros((gp_depth,gp_xrange))          #Zeta'(z,x') --- vertical vorticity 
GradZp = np.zeros((gp_depth,gp_xrange))          #d Zeta'(z,x')/dx' --- gradient of vertical vorticity at y'=0                                    
ZZ     = np.zeros((gp_depth,gp_xrange))          #z  vector (m)
ZZabs  = np.zeros((gp_depth,gp_xrange))          #-z  vector (m) --- always positive
XP     = np.zeros((gp_depth,gp_xrange))          #x' vector (m)
for iz in range(gp_depth):
    ZZ[iz,:]     = -dz*gpz[iz]
    ZZabs[iz:]   = dz*gpz[iz]
    XP[iz,:]     = xp_m[:]
    Zeta[iz,:]     = -cor*zeta_max*np.sin(kappa*xp_m)
    ZetaP[iz,:]    = -kappa*cor*zeta_max*np.cos(kappa*xp_m)
    ZetaPP[iz,:]   = -np.power(k_over_sqrt2,2)*Zeta[iz,:]
...
for ts in ts_plot:
    ...
    if os.path.isfile(path_ar):
        ...
        m_star = np.power((N2*ZetaP*ZetaP)/(12*cor*ZZabs),(1./3.))*time
        phi_star = (3./2.)*m_star*ZZ

        phase = phi_star - Zeta*time/2. + np.pi/4.

        ampli = -u_0*sigma_w/np.sqrt(np.abs(3*ZZ*np.power(m_star,3))) 

        phi_star2= - phi_star*ZetaPP/(ZetaP*ZetaP*time)
        phi_star3= 0.5*grad_zeta*kappa*N2*time*time/(cor*m_star*m_star)

        decay = np.exp(phi_star2-np.power(m_star*sigma_w,2)/4.)

        Gammaz  =ii*m_star            -  m_star*ZetaPP/(ZetaP*ZetaP*time)             +  np.power(sigma_w*m_star,2)/(6.*ZZ)  
        Gammazz =ii*(-m_star/(3.*ZZ)) + (m_star/(3*ZZ))*ZetaPP/(ZetaP*ZetaP*time)     - (5./18.)*np.power(sigma_w*m_star/ZZ,2) 

        MM = Gammazz + np.power(Gammaz,2)

        #There would be a (N/f)^2 factor missing for getting the right A
...
#                    im = ax.imshow(4*phi_star2,cmap=colormap,aspect=aspect,vmin=-5,vmax=5)#,vmin=-grad_zeta,vmax=grad_zeta)
#                    im = ax.imshow(np.exp(4*phi_star2),cmap=colormap,aspect=aspect,vmin=0,vmax=10)#,vmin=-grad_zeta,vmax=grad_zeta)
                    im = ax.imshow(np.exp(phi_star3),cmap=colormap,aspect=aspect,vmin=0,vmax=10)#,vmin=-grad_zeta,vmax=grad_zeta)
                elif(field==12):
#                    im = ax.imshow(4*phi_star2,cmap=colormap,aspect=aspect,vmin=-5,vmax=5)#,vmin=-grad_zeta,vmax=grad_zeta)                                         
#                    im = ax.imshow(np.exp(4*phi_star2),cmap=colormap,aspect=aspect,vmin=0,vmax=10)#,vmin=-grad_zeta,vmax=grad_zeta)                                          
                    im = ax.imshow(LAR,cmap=colormap,aspect=aspect,vmin=-0.05,vmax=0.05)#,vmin=-grad_zeta,vmax=grad_zeta) 
...
#                    im=ax.contour(np.exp(phi_star3),levels=[5,10,20],colors='k')#,colors='k')
                    im=ax.contour(4*phi_star2,levels=[-5,-1,-0.5,0,0.5,1,5],colors='k')#,colors='k')
