...
if not os.path.exists('plots/'+run+'/bu/'):
    os.makedirs('plots/'+run+'/bu/')
...
for ts in range(ts_min,ts_max):
    ...
    if os.path.isfile(path_lar):
        ...
        Bu = (N2/f2)*np.power( (lambda_v/(lambda_h*1000)),2)
        ...
        if(plot_slice==1):
            ...
            for idx,ax in enumerate(grid):
                ...
                im = ax.imshow(Bu,cmap=colormap,vmin=bu_min,vmax=bu_max,aspect=aspect)
#                im = ax.imshow(Bu,cmap=colormap,aspect=aspect)
...
#                cbar.ax.set_yticklabels(ticks)
                ...
                time_title = '%.1f' % time        
                ax.set_title(r'Bu, $t =$ '+time_title+' inertial periods',fontsize=12)
...
#                plt.show()
                ...
                plt.savefig('plots/'+run+'/bu/bu_t'+zeros_ts+str(ts)+'.png',bbox_inches='tight')


make_gif = 'convert -limit thread 1 -delay 1 -loop 0 plots/'+run+'/bu/*.png plots/'+run+'/bu/bu.gif'
