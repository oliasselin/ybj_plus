...
#        phiz  = ii*( N2*beta*beta*np.power(time,3)/(36*cor*m_star*m_star*bigD) + (2./3.)*m_star ) - np.power(sigma_w*m_star,2)/(6.*bigD)
#        phizz = ii*( N2*beta*beta*np.power(time,3)/(108*cor*np.power(m_star*bigD,2)) + (2./9.)*m_star/bigD ) - (5./18.)*np.power(sigma_w*m_star/bigD,2)
...
#                    im = ax.imshow(Zprime,cmap=colormap,aspect=aspect)
                    im = ax.imshow(beta*XP,cmap=colormap,aspect=aspect)
