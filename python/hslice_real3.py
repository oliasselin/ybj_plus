...
run = 'new_input/ml2/'#'strain/N2_1e-5_y_n2-4_nodisp'#'attempt3_ro50'#'ml_100'                                                                                                                                   
...
ts_list=[200]#np.arange(0,131,10)                                                                                                                                                     
wke_min=0.
wke_max=0.005
vort_min=-0.3
vort_max=0.3
...
for ts in ts_list:
    ...
    if(field_to_plot=='strain'):
        path_strain  = scratch_location+folder+run+'/output/slice2htop8 0.dat'
        g_strain = np.flipud(np.reshape(np.loadtxt(path_strain),(hres,hres)))

        if(leif_field!=1):
            g_strain = np.fliplr(g_strain)

        field = g_strain


    if(field_to_plot=='vort_gradient'):
        ...
        print('Vort. grad. at origin: ',field[int(hres/2),int(hres/2)])
...
#Produce the ncases x nts multiplot
        ...
        for idx,ax in enumerate(grid):
            ...
            if(field_to_plot=='vort'):
                ax.set_title(r'Vorticity $\zeta/f$',fontsize=12)
                im = ax.imshow(field,cmap=colormap,vmin=vort_min,vmax=vort_max)   

            if(field_to_plot=='psi'):
                ax.set_title(r'Streamfunction $\psi$ (m$^2$/s)',fontsize=12)
                im = ax.imshow(field,cmap=colormap)#,vmin=vort_min,vmax=vort_max)

            if(field_to_plot=='strain'):
                ax.set_title(r'Strain $\frac{1}{2}(v_x+u_y)/f$',fontsize=12)
                im = ax.imshow(field,cmap=colormap,vmin=vort_min,vmax=vort_max)   
                

            if(field_to_plot=='vort_gradient'):
                ax.set_title(r'Vort. grad. $|\nabla \zeta|$',fontsize=12)
                im = ax.imshow(field,cmap=colormap,vmin=0,vmax=beta_max)
            ...
            if(show_xyp==1):
                ...
                ax.text(hres/2+hres/25,hres/2+hres/8,r"$x'$",rotation='horizontal',horizontalalignment='center',verticalalignment='center', fontsize=15,color=color)
                ax.text(hres/2+hres/25,hres/2-hres/10,r"$y'$",rotation='horizontal',horizontalalignment='center',verticalalignment='center', fontsize=15,color=color)            
...
#            cbar = grid.cbar_aes[0].colorbar(im)
