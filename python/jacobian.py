...
field_to_plot='grad_zeta' #vort, u or v
vloc='htop'
term_max = 5e-7
make_gif=0
plot_slice=1
...
run = 'jacobian/dipole'#'attempt3_ro50'#'ml_100'                                                                                                                                   
...
ts_list=[0]#np.arange(0,131,10)                                                                                                                                                     
wke_min=0.
wke_max=0.005

vort_levels=[-0.2, -0.1, 0. ,0.1, 0.2]
psi_levels= [-6000,-3000,0.,3000,6000]
...
if(compute_correlation==1):
    ...
    g_p = np.loadtxt(path_p)

    path_z  = scratch_location+folder+run+'/output/slice2htop7 0.dat'
    g_z = np.loadtxt(path_z)

    print("Correlation between psi and zeta",np.corrcoef(g_p,g_z))

    print("average vorticity = ",1.24e-4*np.average(np.absolute(g_z)))
...
for ts in ts_list:
    ...
    if(field_to_plot=='jacobian'):

        path  = scratch_location+folder+run+'/output/slice2htop9 0.dat'
        field = np.flipud(np.reshape(np.loadtxt(path),(hres,hres)))

        if(leif_field!=1):
            field = np.fliplr(field)

        print("ave |J| = ",np.average(np.absolute(field)))
    ...
    if(field_to_plot=='U'):
        ...
        field = np.sqrt(np.square(g_u)+np.square(g_v))
        print("ave |U| = ",np.average(field))

        if(leif_field!=1):
            field = np.fliplr(field)


    if(field_to_plot=='grad_zeta'):

        path_zx  = scratch_location+folder+run+'/output/slice2htop4 0.dat'
        g_zx = np.flipud(np.reshape(np.loadtxt(path_zx),(hres,hres)))

        path_zy  = scratch_location+folder+run+'/output/slice2htop5 0.dat'
        g_zy = np.flipud(np.reshape(np.loadtxt(path_zy),(hres,hres)))

        if(leif_field!=1):
            g_zx = np.fliplr(g_zx)
            g_zy = np.fliplr(g_zy)

        field =np.sqrt(np.square(g_zx)+np.square(g_zy))
        print("ave |nabla(Z)| = ",np.average(field))
...
#Produce the ncases x nts multiplot
        ...
        for idx,ax in enumerate(grid):
            ...
            if(field_to_plot=='jacobian'):
                ax.set_title(r'$J(\psi,\zeta)$, s$^{-2}$',fontsize=12)
                im = ax.imshow(field,cmap=colormap,vmin=-5e-10,vmax=5e-10)#,vmin=wke_min,vmax=wke_max)

            if(field_to_plot=='U'):
                ax.set_title(r'$\sqrt{U^2+V^2}$ (m/s)',fontsize=12)
                im = ax.imshow(field,cmap=colormap,vmin=0,vmax=0.5)#,vmin=wke_min,vmax=wke_max)

            if(field_to_plot=='grad_zeta'):
                ax.set_title(r'$|\nabla \zeta|$, (ms)$^{-1}$',fontsize=12)
                im = ax.imshow(field,cmap=colormap,vmin=0,vmax=5e-9)#,vmin=wke_min,vmax=wke_max)
            ...
            if(show_origin==1):
                ax.scatter(hres/2,hres/2,color='y')
...
#            cbar = grid.cbar_axes[0].colorbar(im)
            ...
            if(show_contours==1):
                ...
                im=ax.contour(g_psi,levels=psi_levels,colors='g')#,colors='k')                                                                                                         
