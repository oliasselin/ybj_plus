...
ts_plot=np.arange(0,161,1)#[50,100,150,200]
...
#    plt.plot(kmt[:,0],pred[:,5],'-g',label=r"$k = - 0.5 (\zeta_{x'}(\bar{x'},\bar{y'})/\alpha) (1-\exp(\alpha t))$",linewidth=2.)
    ...
    plt.ylim((0.,0.0015))
    plt.title(r"Horizontal wavenumber, $\kappa y'=$"+str(np.around(kappa_yp,2)))
...
#        plt.plot(kmtp[:,0,point],kmtp[:,1,point],label='x='+str(kmxa_km[point])+' km, z ='+str(kmza_m[point])+' m.')
    ...
    plt.ylim((0.,0.01))
    plt.title(r"Vertical wavenumber, $\kappa y'=$"+str(np.around(kappa_yp,2)))
