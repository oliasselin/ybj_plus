...
if not os.path.exists('plots/'+run+'/lv/'):
    os.makedirs('plots/'+run+'/lv/')
    ...
    if os.path.isfile(path_lar):
        ...
        if(plot_slice==1):
            ...
            for idx,ax in enumerate(grid):
                ...
                im = ax.imshow(lambda_v,cmap=colormap,vmin=lv_min,vmax=lv_max,aspect=aspect)
...
#                cbar.ax.set_yticklabels(ticks)
                ...
                time_title = '%.1f' % time        
                ax.set_title(r'$\lambda_v$ (m), $t =$ '+time_title+' inertial periods',fontsize=12)
...
#                plt.show()
                ...
                plt.savefig('plots/'+run+'/lv/lv_t'+zeros_ts+str(ts)+'.png',bbox_inches='tight')


make_gif = 'convert -limit thread 1 -delay 1 -loop 0 plots/'+run+'/lv/*.png plots/'+run+'/lv/lv.gif'
