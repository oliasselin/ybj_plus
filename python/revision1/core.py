    ...
    for res in res_list:
        for u in u_list:
            for dt in dt_list:
                ...
                path_cexp  = 'data/'+folder+'cexp_'+run+'.dat'            
                path_cexp2 = 'data/'+folder+'cexp2_'+run+'.dat'            
                if os.path.isfile(path_core):

                    core  = np.loadtxt(path_core)
                    cexp  = np.loadtxt(path_cexp)
                    cexp2 = np.loadtxt(path_cexp2)
...
#                    plt.plot(error[:,0],error[:,2]*100,'+g',markersize=5,label='YBJ$^+$')
                    ...
                    plt.plot(cexp[:,0],cexp[:,1],'--b',linewidth=0.7)#,markersize=7,markevery=20,label='BQ')                    
                    plt.plot(cexp[:,0],cexp[:,2],'--g',linewidth=0.7)#,markersize=7,markevery=20,label='BQ')

                    plt.plot(cexp2[:,0],cexp2[:,1],':b',linewidth=0.7)#,markersize=7,markevery=20,label='BQ')                    
                    plt.plot(cexp2[:,0],cexp2[:,2],':g',linewidth=0.7)#,markersize=7,markevery=20,label='BQ')
